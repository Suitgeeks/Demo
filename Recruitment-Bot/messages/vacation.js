...
// Create library
const library = new builder.Library('Vacation');
...
library.dialog("Main", [
    ...
    function(session, args, next) {       
        ...
        if (session.conversationData['jobType'] === 'Part-time') {
            session.send(responses.vacationPartTime);
        } else {
            session.send(responses.vacationFullTime);
        }
        session.endDialog();
    }
]);  
...
library.dialog("JobType", [
    function(session, args, next) {       
        builder.Prompts.choice(session, responses.vacationJobType, 
...
.cancelAction('cancelAction', responses.FurtherAssistance, {  ///////////////////// TO DO
    matches: ['Cancel'],
    onSelectAction: (session, args, next) => {
      next();
      session.replaceDialog(0,'/',args);
    },
    confirmPrompt: responses.QuitingDialogQuestion
});  
